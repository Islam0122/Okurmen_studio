...
def sent_prompt_and_get_response_2(msg: str):
    ...
    message = (
        ...
        "Ответьте на этот вопрос, используя информацию о нашей студии Окурмэн Студия. "
...
def sent_prompt_and_get_response_3(msg: str):
    access_token = get_access_token()

    # Формируем сообщение для ИИ с учётом темы пользователя
    message = (
        f"Пользователь задал тему: '{msg}'. "
        f"Создай что-то вдохновляющее на основе этой темы. Это может быть одно из следующих: "
        f"1. Стихотворение, отражающее настроение или смысл темы. "
        f"2. Текст песни, включая куплеты, припев и, если нужно, мост. "
        f"3. Концепция музыкального произведения, включая жанр, настроение и ключевые инструменты."
        f"Выбери наиболее подходящий формат на основе темы и постарайся сделать это уникальным и запоминающимся."
    )
